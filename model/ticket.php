...
class Ticket {
    ...
    public function table_ticket(): array{
         $sql = "
                SELECT 
                A.id, B.id_productservice as id_producto, 
                date(A.fecha) as fecha_pedido, 
                case when upper(C.USUARIO) is null then 'SALIDA INSUMOS' else upper(C.USUARIO) end as usuario,
                case when E.apellidos is null then 'INVENTARIO' else concat(E.nombres,' ',E.apellidos) end as cliente, 
                GROUP_CONCAT('# ',D.nombre SEPARATOR ' </br> ') as productos, GROUP_CONCAT('S/.',B.precio,' x ',B.cantidad SEPARATOR ' </br> ') as precioxcant,
                ...
                where date(A.fecha) between '2025-10-01' and '2025-10-31' and D.id_sucursal=5 and A.cliente>0
