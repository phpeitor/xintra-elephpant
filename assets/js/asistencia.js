(() => {
    ...
    fetch(`controller/asistencia.php?action=activity${query}`)
      ...
      .then((json) => {
        ...
        activity.innerHTML = json.data.length
          ? json.data.map((item) => `<li class="attendance-activity-item ${item.tipo === "ENTRADA" ? "is-entry" : "is-exit"}"><span class="attendance-activity-dot"></span><div><strong>${item.tipo === "ENTRADA" ? "Entrada" : "Salida"}</strong><span>${item.usuario}</span><time>${new Date(item.start).toLocaleString("es-PE")}</time></div></li>`).join("")
  ...
  const buildCalendar = () => {
    calendar = new FullCalendar.Calendar(calendarElement, {
      ...
      eventClick: (info) => alertify.message(`${info.event.extendedProps.tipo}: ${info.event.extendedProps.usuario}<br>${info.event.start.toLocaleString("es-PE")}`),
    });
    calendar.render();
  };
